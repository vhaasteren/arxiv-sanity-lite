...
def add_entry_to_tags(username, new_tag, paper_id, whitelist_tags=[], whitelist_papers=[]):
    with get_tags_db(flag='c', autocommit=True) as tags_db:  # Open db in create mode to allow writes
        ...
        if len(whitelist_tags) > 0 and new_tag not in whitelist_tags:
            return

        if len(whitelist_papers) > 0 and paper_id not in whitelist_papers:
            return
...
def find_arxiv_papers_by_dois(dois, chunk_size=900, uncompressed_db_fn='uncompressed_papers_db.sqlite'):
    uncompressed_db = sqlite3.connect(uncompressed_db_fn)
    arxiv_ids = []
    ...
    for doi_chunk in chunked(dois, chunk_size):
        placeholders = ', '.join(['?'] * len(doi_chunk))  # Create placeholder for each DOI in the chunk
        sql_query = f'''
        SELECT arxiv_id, json_extract(data, '$.arxiv_doi')
        FROM papers
        WHERE json_extract(data, '$.arxiv_doi') IN ({placeholders})
        '''
        cursor = uncompressed_db.execute(sql_query, doi_chunk)
        arxiv_ids.extend([(row[0], row[1]) for row in cursor.fetchall()])

    uncompressed_db.close()

    return arxiv_ids

def compile_tags(arxiv_ids, zotlib):
    tags = defaultdict(set)

    for ii, (arxiv_id, doi) in enumerate(arxiv_ids):

        zotlibitem = [zli for zli in zotlib['items'] if 'DOI' in zli and zli['DOI'] == doi][0]
        item_tags = zotlibitem['tags']

        #print(item_tags)
        for item_tag in item_tags:
            tags[item_tag['tag']].update({arxiv_id})

    return tags

def compile_new_zotero_tags(username, zotlib, new_tag='zotero', uncompressed_db_fn='uncompressed_papers_db.sqlite', doi_chunk_size=900):

    doi_list = [zl_item['DOI'] for zl_item in zotlib['items'] if 'DOI' in zl_item]

    arxiv_doi_ids = find_arxiv_papers_by_dois(doi_list, chunk_size=doi_chunk_size, uncompressed_db_fn=uncompressed_db_fn)

    tags = compile_tags(arxiv_doi_ids, zotlib)
    tags[new_tag].update(set([a[0] for a in arxiv_doi_ids]))

    return tags
    #for ii, arxiv_id in enumerate(arxiv_doi_ids):
    #    add_entry_to_tags(username, new_tag, arxiv_id)
    #    print(f"Adding: {ii}: {username}, {new_tag}, {arxiv_id}")
